...
nmissedoraclevotes=20  # checks back the missed oracle votes n blocks from blockheight, a value higher than 100 is not supported by terrad
...
while true; do
    ...
    if [ "$result" != 0 ]; then
        ...
        if [ $nmissedoraclevotes -eq 0 ]; then pctvotes="1.0"; else pctvotes=$(echo "scale=2 ; 1 - (($nmissedend - $nmissedstart) / $nmissedoraclevotes)" | bc); fi
